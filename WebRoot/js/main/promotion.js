...
$.fn.getAllProduct = function(options){
    ...
    this.bind("click", function(e){
    	if($("#orderTotal").attr("checked") == "checked"){
    		chooseProduct();
    	}
    });
  //默认传递参数
    ...
    var submit = function(){
    	var checkInput = $(dialogTbody).find("tr input:checked");
    	var curTr = $(checkInput).closest("tr");
    	var name = $(curTr).find("input[name='name']").val();
    	$(resultId).val(checkInput.val());
		$(resultName).val(name);
		$("#productId").val(checkInput.val());
...
// 生成单号的方法
function CreatePromotionNum(type) {
	var now = new Date();
...
$("#beginDate").datetimepicker({
	...
	timepicker : false,
	format : 'Y-m-d',
	minDate : '-1970/01/01', // 表示最小的日期只能选择今天
});
