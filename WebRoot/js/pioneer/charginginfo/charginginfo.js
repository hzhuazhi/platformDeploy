...
function pageNums(){
    ...
    this.pageNo = 1;
    charginginfo.querData(pageNo,lenth)
...
var charginginfo = {
	...
	querData:function(pageNo,lenth){
		...
		var url = basePath+"charginginfo/dataList.do";
		idea.base.request(data,url,function(data){
			if(data.rows.length>0){
				...
				var shtml="";
			 	for(var i=0;i<dataList.length>0;i++){
			 	 	shtml +="<tr onmouseover='charginginfo.trMouseOver(this)' onmouseout='charginginfo.trMouseOut(this)' >"
			 		...
			 		shtml +="<td style='text-align: center; white-space:nowrap; vertical-align:middle; '><a title='Update' class='dataTableBtn dataTableResetBtn'href='javascript:charginginfo.updateData(1,"+dataList[i].id+",\""+pageNo+"\",\""+lenth+"\")'>编辑</a>";
			 	  	shtml +="&nbsp;";
			 	  	shtml +="<a title='Delete'class='dataTableBtn dataTableDeleteBtn' href='javascript:charginginfo.updateData(2,"+dataList[i].id+")'>删除</a>";
			 	  	shtml +="</tr>"
			 	}
			...
			charginginfo.initPageBar(pageNo,totalPage,pageSize,rowCount);
			idea("#pageNo2").val('');
			idea("#lenth2").val('');
		});
	},
...
//		this.hidediv();
		idea("#showUpMsg").html("");
		if(type==1){
			var url = basePath+"jsp/tvdeploy/charginginfo/charginginfoEdit.jsp?id="+id+"&pageNo2="+pageNo2+"&lenth2="+lenth2;
			...
			if(window.confirm("确认：是否删除?")){
				...
				var url = basePath+"charginginfo/doUpdate.do";
				idea.base.request(data,url,function(data){
					if(data.msg=="请重新登录~"){
						alert("页面超时，请重新登录");
	...
	//加载编辑数据
	editChargingInfo:function(id){
		idea("#id").val(id);
		var url = basePath+"charginginfo/showEdit.do";
		var data = {
				"id":id
	 	};
		idea.base.request(data,url,function(data){
...
//		this.hidediv();
	...
	doUpdate:function(){
		...
		idea.base.request(data,url,function(data){
			idea("#showMessage").html("<font color='red'>"+data.msg+"</font> ");
			if(data.msg=="修改成功~"){
				var url = basePath+"jsp/tvdeploy/charginginfo/charginginfoIndex.jsp";
				alert(data.msg);
				window.location.href=url;
			}else{
				alert("页面超时，请重新登录");
	...
	addExcel:function(){
        ...
        var url = basePath+"charginginfo/addExcel.do";
	...
	//跳转到新增页面
	addUrl:function(){
			var url = basePath+"jsp/tvdeploy/charginginfo/charginginfoAdd.jsp";
			window.location.href=url;
	},
	...
	//新增数据
	add:function(){
		var url = basePath+"charginginfo/add.do";
		var sdkNum =  idea("#sdkNum").val();
		var chargingPoint =  idea("#chargingPoint").val();
        var myChargingPoint =  idea("#myChargingPoint").val();
        var chargingName =  idea("#chargingName").val();
	...
	initPageBar: function(pageNo, totalPage,pageSize,rowCount) {
		...
		pageInstance = idea.fn.jpagebar({
			...
			onClickPage : function(pageIndex,pageSize) {
				...
				pageNo = pageIndex;
				pageSize2=pageSize;
				charginginfo.querData(pageIndex, pageSize2);
			}
		});
