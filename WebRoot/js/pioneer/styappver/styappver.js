...
function pageNums(){
    ...
    this.pageNo = 1;
    styappver.querData(pageNo,lenth)
...
//				"limitType":limitType,
		...
		var url = basePath+"styappver/dataList.do";
		idea.base.request(data,url,function(data){
			if(data.rows.length>0){
				...
				var shtml="";
			 	for(var i=0;i<dataList.length>0;i++){
			 	 	shtml +="<tr onmouseover='styappver.trMouseOver(this)' onmouseout='styappver.trMouseOut(this)' >"
			 		...
			 		shtml +="<td style='text-align: center; white-space:nowrap'>"+dataList[i].givenTime+"</td>";
			 		...
			 		shtml +="<td style='text-align: center; white-space:nowrap; vertical-align:middle; '><a title='Update' class='dataTableBtn dataTableResetBtn'href='javascript:styappver.updateData(1,"+dataList[i].id+",\""+pageNo+"\",\""+lenth+"\")'>编辑</a>";
			 	  	shtml +="&nbsp;";
			 	  	shtml +="<a title='Delete'class='dataTableBtn dataTableDeleteBtn' href='javascript:styappver.updateData(2,"+dataList[i].id+")'>删除</a>";
			 	  	shtml +="</tr>"
			 	}
			...
			styappver.initPageBar(pageNo,totalPage,pageSize,rowCount);
			idea("#pageNo2").val('');
			idea("#lenth2").val('');
		});
	},
	...
	updateData:function(type,id,isOnline,pageNo2,lenth2){
		...
		idea("#showUpMsg").html("");
		if(type==1){
			var url = basePath+"jsp/tvdeploy/styappver/styappverEdit.jsp?id="+id+"&pageNo2="+pageNo2+"&lenth2="+lenth2;
			...
			if(window.confirm("确认：是否删除?")){
				...
				var url = basePath+"styappver/doUpdate.do";
				idea.base.request(data,url,function(data){
					if(data.msg=="请重新登录~"){
						alert("页面超时，请重新登录");
	...
	//加载编辑数据
	editStyappver:function(id){
		idea("#id").val(id);
		var url = basePath+"styappver/showEdit.do";
		var data = {
				"id":id
	 	};
		idea.base.request(data,url,function(data){
				...
				idea("#givenTime").val(data.givenTime);
...
//		this.hidediv();
	...
	doUpdate:function(){
		...
		var givenTime=idea("#givenTime").val();
		...
		idea.base.request(data,url,function(data){
			idea("#showMessage").html("<font color='red'>"+data.msg+"</font> ");
			if(data.msg=="修改成功~"){
				var url = basePath+"jsp/tvdeploy/styappver/styappverIndex.jsp";
				alert(data.msg);
				window.location.href=url;
			}else{
				alert("页面超时，请重新登录");
	...
	//跳转到新增页面
	addUrl:function(){
			var url = basePath+"jsp/tvdeploy/styappver/styappverAdd.jsp";
			window.location.href=url;
	},
	//新增数据
	add:function(){
		var url = basePath+"styappver/add.do";
		var cpNum=idea("#cpNum").val();
		var appNum=idea("#appNum").val();
		var channelNum=idea("#channelNum").val();
		var batchNum=idea("#batchNum").val();
	...
	initPageBar: function(pageNo, totalPage,pageSize,rowCount) {
		...
		pageInstance = idea.fn.jpagebar({
			...
			onClickPage : function(pageIndex,pageSize) {
				...
				pageNo = pageIndex;
				pageSize2=pageSize;
				styappver.querData(pageIndex, pageSize2);
			}
		});
