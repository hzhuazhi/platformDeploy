...
var account = {
    //地址
    url:{
        list_url : ctx + '/withdrawagentcheck/list.do',
        dataList_url : ctx + "/withdrawagentcheck/dataList.do",
        add_url : ctx+ "/withdrawagentcheck/add.do",
        update_url : ctx+ "/withdrawagentcheck/update.do",
        queryId_url: ctx+ "/withdrawagentcheck/getId.do",
        delete_url: ctx+ "/withdrawagentcheck/delete.do",
        manyOperation_url: ctx+ "/withdrawagentcheck/manyOperation.do"
    },
    //列表显示参数
    list:[
        {"data":"agentName",},
        ...
        {"data":"id",
            "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                ...
                if (oData.withdrawStatus == 1){
                    html = '<a class = "dataTableBtn dataTableDeleteBtn " href="'+ctx+'/withdrawagentcheck/jumpUpdate.do?id='+oData.id+'"> 未审核 </a>'
                }else if (oData.withdrawStatus == 2){
                    html = '<a class = "dataTableBtn dataTableDeleteBtn " href="'+ctx+'/withdrawagentcheck/jumpUpdate.do?id='+oData.id+'"> 未审核 </a>'
                }else if (oData.withdrawStatus == 3){
                    html = '已审核';
