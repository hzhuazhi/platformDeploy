				...
				<li style="border-top: none;">
					...
					<div class="formCtrlDiv">
						<select id="type" name="roleId"> 
							 <c:forEach items="${rloeMenu}" var="rl" >
 	                           <c:choose>   
                                  <c:when test="${rl.isOk=='0' && rl.roleId eq dl.roleId}"> 
                                      <option value="${rl.roleId}" selected>${rl.roleName}</option>
                                  </c:when> 
                                  <c:when test="${rl.isOk=='0' && rl.roleId != dl.roleId}">  
                ...
                <li style="border-top: none;">
					...
					<div class="formCtrlDiv">
						<select id="isEnable" name="isEnable">
							<option value="0" ${dl.isEnable ==2 ? 'selected': ''}>启用</option>
							<option value="1" ${dl.isEnable ==1 ? 'selected': ''}>禁用</option>
						</select>
					</div>
				</li>
				<li>
		...
		messages: {
			...
			submitHandler : function() {
				var formData = $("#addSupplierForm").serialize();
				 $.ajax({
						url : ctx+ "/system/account/update.do",
						type : 'post',
						dataType : 'json',
						data :formData,
